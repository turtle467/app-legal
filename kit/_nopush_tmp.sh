...
if git remote get-url origin >/dev/null 2>&1; then
  echo "(push は保留のためスキップ)"
else
  echo SKIP gh repo create "$GH_USER/$REPO" --public --source=. --remote=origin --push
fi

# GitHub Pages を有効化(既に有効なら無視)
echo SKIP gh api -X POST "repos/$GH_USER/$REPO/pages" -f "source[branch]=main" -f "source[path]=/" >/dev/null 2>&1 \
  || true
